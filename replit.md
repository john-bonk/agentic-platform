...
-   **Demo Control Walkthrough (CTL-003)**: The Segregation of Duties control (CTL-003, manual PBC) is excluded from auto-progression and provides a full manual walkthrough experience. Each of the 8 steps shows SoD-specific detailed data tables (control objectives, population ingestion, stratified sampling, evidence matrix, document classification, attribute evaluation results, effectiveness conclusions with identified exceptions). Action buttons are functional: clicking a primary action (Approve/Confirm/Finalize) advances the step to "complete" in the store, collapses it, and auto-expands the next step. Step advancement is monotonic — completed steps cannot be regressed. The readiness step shows SoD-specific readiness rows (SoD policy, conflict matrix, user access listing, role-permission mapping). Non-demo controls auto-expand their active step; demo control starts fully collapsed.
